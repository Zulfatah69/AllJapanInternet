...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <div class="flex items-center gap-2 mb-2 text-sm font-medium text-slate-500">
                ...
                <span>/</span>
                <span class="text-slate-800">Edit Promo</span>
            </div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Edit Banner Promo: {{ $promo->judul }}</h1>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <form action="{{ route('promos.update', $promo->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label class="block mb-2 text-sm font-semibold text-slate-700">Judul Promo</label>
                <input type="text" name="judul" value="{{ old('judul', $promo->judul) }}" class="w-full border-slate-200 rounded-xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Contoh: Promo Spesial Ramadhan" required>
            </div>

            <div>
                <label class="block mb-2 text-sm font-semibold text-slate-700">Banner Promo</label>
                <x-image-upload name="gambar" :existingImage="$promo->gambar" />
