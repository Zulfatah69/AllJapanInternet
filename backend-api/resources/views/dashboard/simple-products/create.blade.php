...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            ...
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Tambah Produk WiFi</h1>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <form action="{{ route('simple-products.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label class="block mb-2 text-sm font-semibold text-slate-700">Nama Produk</label>
                    <input type="text" name="nama" value="{{ old('nama') }}" class="w-full border-slate-200 rounded-xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Contoh: Router WiFi 5G">
                </div>

                <div>
                    <label class="block mb-2 text-sm font-semibold text-slate-700">Nama Produk (Inggris)</label>
                    <input type="text" name="nama_en" value="{{ old('nama_en') }}" class="w-full border-slate-200 rounded-xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Example: 5G WiFi Router">
                </div>
            </div>
