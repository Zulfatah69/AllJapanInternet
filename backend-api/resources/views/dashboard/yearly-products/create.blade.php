...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            ...
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Tambah Produk Tahunan</h1>
        </div>
    </div>
    ...
    <form action="{{ route('yearly-products.store') }}" method="POST" enctype="multipart/form-data" class="pb-20">
        @csrf
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nama" value="{{ old('nama') }}" class="w-full border-slate-200 rounded-xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Contoh: Softbank 100GB Tahunan">
                        </div>

                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700">Nama Produk (Inggris)</label>
                            <input type="text" name="nama_en" value="{{ old('nama_en') }}" class="w-full border-slate-200 rounded-xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Example: Softbank 100GB Yearly">
                        </div>

                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700">Deskripsi Produk</label>
                            <textarea name="deskripsi" rows="4" class="w-full border-slate-200 rounded-xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">{{ old('deskripsi') }}</textarea>
                        </div>

                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700">Deskripsi Produk (Inggris)</label>
                            <textarea name="deskripsi_en" rows="4" class="w-full border-slate-200 rounded-xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">{{ old('deskripsi_en') }}</textarea>
                        </div>

                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700">Gambar Produk (Thumbnail)</label>
                            <x-image-upload name="thumbnail" />
                        </div>
                        
                        <div class="flex items-center gap-3 pt-4 border-t border-slate-100">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="best_seller" value="1" class="sr-only peer" {{ old('best_seller') ? 'checked' : '' }}>
                            ...
                            <template x-for="(variant, index) in variants" :key="variant.id">
                                <div class="relative bg-slate-50 border border-slate-200 rounded-xl p-5 pt-8 transition-all">
                                    ...
                                    <div class="mb-5">
                                        ...
                                        <input type="text" :name="`variants[${index}][nama]`" x-model="variant.nama" placeholder="Contoh: 35GB" class="w-full sm:w-1/2 border-slate-200 rounded-xl px-4 py-2.5 focus:ring-indigo-500 focus:border-indigo-500 transition-colors bg-white">
                                    </div>
                                            ...
                                            <template x-for="period in periods" :key="period">
                                                <div>
                                                    ...
                                                    <div class="relative">
                                                        ...
                                                        <input type="number" :name="`variants[${index}][periods][${period}]`" class="w-full border-slate-200 rounded-lg pl-8 pr-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500 transition-colors bg-white">
                                                    </div>
                                                </div>
                                            </template>
                                        </div>
                            ...
                            @foreach($methods as $index => $method)
                                <div class="grid grid-cols-2 gap-4 items-center bg-slate-50 p-4 rounded-xl border border-slate-100">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="payment_methods[{{ $index }}][additional_price]" value="{{ old('payment_methods.'.$index.'.additional_price', 0) }}" class="w-full border-slate-200 rounded-lg px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors bg-white">
