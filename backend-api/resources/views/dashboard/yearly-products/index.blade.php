...
<div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
    <div>
        <h1 class="text-3xl font-black text-slate-900 tracking-tight mb-1">Produk Tahunan</h1>
        <p class="text-slate-500 font-medium">Kelola produk internet tahunan</p>
    </div>
    <a href="{{ route('yearly-products.create') }}" class="inline-flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-3 rounded-xl font-semibold transition-colors shadow-sm shadow-indigo-200 gap-2">
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-8">
    <form action="{{ route('yearly-products.index') }}" method="GET" class="flex flex-col sm:flex-row sm:items-end gap-4">
        <div class="flex-1">
        ...
        <div class="flex-1">
            <label class="block text-sm font-semibold text-slate-700 mb-2">Filter Penyedia Layanan</label>
        ...
        <div class="flex items-center gap-3 w-full sm:w-auto">
            <button type="submit" class="flex-1 sm:flex-none bg-slate-900 hover:bg-slate-800 text-white px-6 py-3 rounded-xl font-semibold transition-colors">
                Terapkan Filter
            </button>
            <a href="{{ route('yearly-products.index') }}" class="flex-1 sm:flex-none text-center bg-slate-100 hover:bg-slate-200 text-slate-700 px-6 py-3 rounded-xl font-semibold transition-colors">
                Reset
            </a>
        </div>
    </form>
        ...
        <table class="w-full">
            <thead class="bg-slate-50 text-left border-b border-slate-100">
                <tr>
                    ...
                    <th class="p-5 text-sm font-semibold text-slate-600 text-center">Status Aktif</th>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="p-5">
                            <div class="flex items-center gap-4">
                                @if($product->thumbnail)
                                    <img src="{{ asset('storage/' . $product->thumbnail) }}" class="w-16 h-16 rounded-xl object-cover border border-slate-200">
                                @else
                                ...
                                @endif
                                <div>
                                    <h2 class="font-bold text-slate-900 mb-1">{{ $product->nama }}</h2>
                        ...
                        <td class="p-5">
                            <div class="flex items-center gap-2">
                                @if($product->provider && $product->provider->logo)
                                    <img src="{{ asset('storage/' . $product->provider->logo) }}" class="w-6 h-6 rounded-md object-cover border border-slate-200" title="{{ $product->provider->nama }}">
                                @endif
                                <span class="font-medium text-slate-700">{{ $product->provider->nama ?? '-' }}</span>
                            </div>
                        </td>
                        <td class="p-5 text-center align-top">
                            <label class="relative inline-flex items-center cursor-pointer" title="Ubah status aktif produk">
                        ...
                        <td class="p-5">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('yearly-products.edit', $product->id) }}" class="p-2 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors" title="Edit">
                                ...
                                <form action="{{ route('yearly-products.destroy', $product->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="p-12 text-center text-slate-400">
                            ...
                            <p class="font-medium">Belum ada produk tahunan ditemukan.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
