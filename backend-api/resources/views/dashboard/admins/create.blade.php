    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <div class="flex items-center gap-2 mb-2 text-sm font-medium text-slate-500">
                ...
                <span>/</span>
                <span class="text-slate-800">Tambah Admin Baru</span>
            </div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Tambah Administrator</h1>
        </div>
    </div>
        ...
        <form action="{{ route('admins.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="w-full border-slate-200 rounded-xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Contoh: John Doe" required>
                </div>

                <div>
                    <label class="block mb-2 text-sm font-semibold text-slate-700">Email Login</label>
                    <input type="email" name="email" value="{{ old('email') }}" class="w-full border-slate-200 rounded-xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Contoh: admin@website.com" required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="password" name="password" class="w-full border-slate-200 rounded-xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Minimal 8 karakter" required>
                </div>

                <div>
                    <label class="block mb-2 text-sm font-semibold text-slate-700">Konfirmasi Password</label>
                    <input type="password" name="password_confirmation" class="w-full border-slate-200 rounded-xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Ketik ulang password" required>
                </div>
            </div>
